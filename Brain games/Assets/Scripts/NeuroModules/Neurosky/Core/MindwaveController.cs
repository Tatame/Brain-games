...
namespace NeuroModules.Neurosky.Core
{
    ...
    public class MindwaveController : MonoBehaviour, IController
    {
        #region Attributes
        ...
        public event Action<int> OnUpdateRawEEG;

        // Triggered when blink data are get from Mindwave.
        public event Action<int> OnUpdateBlink;

        // Settings

        [Header("Connection settings")] [SerializeField, Tooltip("By default: 127.0.0.1")]
        ...
        [SerializeField,
         Tooltip(
             "If it's set to false, you must connect to Mindwave manually using Connect(). Else, automatically try to connect to Mindwave at game start.")]
        ...
        [SerializeField] private bool m_ShowStreamErrors = false;
            ...
            if (m_DataStream.CanRead)
            {
                ...
                foreach (string packet in packets)
                {
                    ...
                    try
                    {
                        ...
                        {
                            ISenseModel model = JsonUtility.FromJson<Data.MindwaveDataModel>(packet);
        ...
        /// If not, emit OnConnect or OnDisconnect event, and change the ConnectedFlag value.
        /// </summary>
